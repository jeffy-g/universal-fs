  ...
  static async expectAsyncError(
    ...
    try {
      ...
      if (expectedType && !(e instanceof expectedType)) {
        throw new Error(`Expected error of type ${expectedType.name} but got ${(e as any).constructor.name}`);
      }

      if (expectedMessage) {
        if (typeof expectedMessage === 'string') {
          expect((e as any).message).toContain(expectedMessage);
        } else {
          expect((e as any).message).toMatch(expectedMessage);
        }
      }
      return e as Error;
    }
